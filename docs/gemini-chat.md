...
- The chat composer has no Search trains button. Gemini can request a search when the user explicitly asks to proceed; the app checks all required fields and unresolved ambiguity before calling the existing train endpoint and opening results on the right. Merely completing the form is not search consent. Manual search remains available in the details panel.
- Chat can select a displayed journey/class or the confirmed recommendation, resolved against actual search results rather than invented IDs. Waitlisted alternatives remain selectable explicitly, with their status visible in the final review; a waitlisted-only set is never chosen as the recommendation.
- After selection, the same chat collects each traveller's full name, age, gender and optional berth preference, then booking email/mobile number. A bounded workflow snapshot preserves these fields across turns. The right-hand panel provides equivalent editable inputs, followed by a complete itinerary/class/availability, traveller/contact and fare/fee review. An explicit final reply such as “Confirm booking” or “Yes, book it” completes the existing local journey-summary flow, without another Gemini call. An earlier yes, question, conditional confirmation or stale review cannot complete it. Corrections invalidate the prior review; duplicate confirmation keeps the same reference.
- Chat cannot issue real tickets, charge money, reserve seats or dispatch services. The final summary explicitly directs ticketing/payment to the provider. No payment credentials or government ID numbers are requested in chat. Search continues using the existing catalogue/RailRadar adapter; generated text is not live railway data.
- Follow-up questions receive a compact snapshot of up to five displayed journey options, including stable IDs, classes and road/rail legs. After selection, traveller names/ages/genders/preferences and booking contact are included in a separately bounded workflow snapshot sent to Gemini. The booking panel discloses this. The text the user voluntarily enters in chat is also sent to Gemini. Payment data and assistance contacts are not sent. Conversation and booking state remain in memory; no durable ticket record or chat log is created.
...
3. **Request limits:** 2,000 characters per message, 16 recent messages, 12,000 history characters, 6,000 result-context characters, 32 KB request body, 1,400 planning output tokens (2,200 while collecting booking details) and a 25-second provider timeout. Workflow context is bounded to eight travellers and 5,000 summary characters. There are no automatic paid retries. The client has a 35-second request timeout and a 40-turn conversation limit. Final confirmation is handled locally, with no paid model call.
...
134 automated tests pass, including chat actions, required-field/ambiguity guards, actual-result/class selection, waitlist safety, traveller/contact validation, keeping captured fields during clarification, current-review fingerprints, conditional/negative consent and idempotent final confirmation, plus the existing auth/extraction/search/cache controls. Typecheck, lint and production build pass.

The new opt-in `scripts/test-checkout-live.mjs` sends six bounded Gemini requests using an operator-signed session and synthetic traveller/contact data. Run with Node `--env-file=.env.local --experimental-strip-types`. It verifies planning without auto-search, “go ahead and serach”, result selection, two travellers, contact details and a pre-confirmation correction. The first cold request timed out; a bounded rerun passed all scenarios. UI clicks/screenshots were not re-tested in this update. Existing unit tests verify final local confirmation independently of model output.
