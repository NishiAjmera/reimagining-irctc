...
export function ContextQuestions({ journey, intent }: { journey: JourneyOption; intent: JourneyIntent }) {
  ...
  return (
    <section className="questions-section" aria-labelledby="questions-title">
      <div><h2 id="questions-title">Quick answers</h2></div>
      <div className="question-list">{suggestedQuestions.slice(0, 3).map((question) => <button key={question} type="button" onClick={() => ask(question)}>{question}<span>→</span></button>)}</div>
