...
export function JourneyCard({journey,index,onChoose,isOtherOption=false,decision}:{journey:JourneyOption;index:number;onChoose:(journey:JourneyOption)=>void;isOtherOption?:boolean;decision?:{label:string;reason:string;tradeoff:string;priorityHits:string[]}}) {
  ...
  const badge=decision?.label??(status==='WAITLIST'?'Waitlisted option':status==='RAC'?'RAC option':isOtherOption?'Additional option':badgeCopy[journey.recommendationType]);
  ...
  return <article className={`journey-card compact-journey-card ${index===0&&!isOtherOption&&status==='CONFIRMED'?'featured':''}`}>
    ...
    {decision?<div className="decision-rationale"><p>{decision.reason}</p>{decision.priorityHits.length?<div>{decision.priorityHits.map(item=><span key={item}>{item}</span>)}</div>:null}<small><b>Trade-off</b> {decision.tradeoff}</small></div>:null}
    ...
    <div id={detailsId} hidden={!expanded} className="compact-expanded-details"><h4>Why this option</h4><ul>{[...(decision?[decision.reason]:[]),...active.reasons,...active.tradeoffs].map((reason,i)=><li key={`${reason}-${i}`}>{reason}</li>)}</ul>{status!=='CONFIRMED'?<p className="availability-explainer"><b>{status==='RAC'?'RAC means you can travel, but a full berth may be shared until confirmation.':'Waitlist means you cannot board unless the ticket confirms.'}</b> Confirmation confidence: {active.classOption.confidence}%. {intentImpact(status)}</p>:null}</div>
  </article>;
}

function intentImpact(status:'CONFIRMED'|'RAC'|'WAITLIST') { return status==='CONFIRMED'?'':status==='RAC'?'This ranks below confirmed options when confirmation matters.':'This stays outside the recommended set when confirmed seats are required.'; }
