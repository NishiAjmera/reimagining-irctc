...
Required: supported origin and destination, explicit departure date, explicit traveller count. For supported towns default journeyMode to complete unless the user explicitly asks for train only. The application will automatically choose practical nearby stations and surface an editable route recommendation; do not ask the user to solve the railhead choice before search. Support bus+train+bus when both ends are towns. For unsupported towns ask the user to select a supported location; do not invent one.
