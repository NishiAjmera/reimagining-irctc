...
**Release 23 published; conversational search and booking review live**

Release 23 was published on 29 August 2026 from Site source `416ceac`, matching GitHub app source `75b4265`, with hosted environment revision 3. Travel-extras discovery was refined following user feedback: the coloured icon panel is replaced by one muted 12px sentence below the results heading, naming optional cabs, food, luggage help and parcels on the confirmation page. Removed repeated notices from manual traveller/payment and chat checkout screens. No card background, border, icon row or extra controls remain; the actual confirmation-page extras are unchanged. All 138 tests, typecheck, lint and production build pass.
...
Validation: 134 automated conversational-flow tests (138 after extras discovery checks), typecheck, lint and production build pass. A six-turn live Gemini regression verified search consent, typo handling, recommended selection, two-passenger details, contact capture and “do not book yet” correction. The first live attempt timed out; the bounded rerun passed. A follow-up question no longer discards supplied traveller details. Release 23 is live; public access and demo-account chat protection are unchanged.
...
Release 24 is live. The journey-results flow now keeps an editable “What we understood” summary visible, automatically recommends practical nearby-station routes, differentiates the best three options by overall fit, savings and arrival time, and explains ranking trade-offs using the traveller’s actual priorities. Priorities rerank the existing option set without restarting search, RAC/waitlist states include contextual guidance, and town-to-town bus + train + bus routes remain supported.

Conversational checkout now pauses at “Your journey is ready” before collecting traveller details. Final approval accepts natural confirmations such as “looks good”, “book it”, “confirm” and “yes please”, then opens a payment-method screen. After completing the payment flow, chat remains active with grounded context for Ola/Uber cabs, Swiggy food delivery, ₹80-per-bag station luggage assistance and parcel services. Release validation covers 148 automated tests, typecheck, lint and production build.
