...
Travel-extras discovery was refined following user feedback: the coloured icon panel is replaced by one muted 12px sentence below the results heading, naming optional cabs, food, luggage help and parcels on the confirmation page. Removed the repeated notices from manual traveller/payment and chat checkout screens. No card background, border, icon row or extra controls remain; the actual confirmation-page extras are unchanged. All 138 tests, typecheck, lint and production build pass. This refinement remains local pending public publishing approval, together with the conversational checkout changes below.
