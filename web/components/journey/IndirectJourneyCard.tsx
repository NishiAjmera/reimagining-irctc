...
export function IndirectJourneyCard({ journey, onChoose }: { journey: JourneyOption; onChoose: (journey: JourneyOption) => void }) {
  ...
  const statusText = status === 'CONFIRMED' ? 'Confirmed on both legs' : status === 'RAC' ? `RAC ${journey.classOption.position}` : `WL ${journey.classOption.position}`;
  ...
  return <article className="indirect-card" aria-labelledby={`journey-${journey.id}`}>
    ...
    <div className="indirect-footer"><div className="indirect-facts"><span><small>Class</small><strong><TermTip code={journey.classOption.code} /></strong></span><span><small>Fare / traveller</small><strong>₹{journey.classOption.fare.toLocaleString('en-IN')}</strong></span><span><small>Availability</small><strong className={`status ${status.toLowerCase()}`}>{status === 'CONFIRMED' ? <Check size={15} /> : <TriangleAlert size={15} />}{statusText}</strong></span></div><button className="choose-button" type="button" onClick={() => onChoose(journey)}>Choose connection <ArrowRight size={17} /></button></div>
  </article>;
}
